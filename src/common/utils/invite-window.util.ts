import { INVITE_WINDOW_END, INVITE_WINDOW_START } from '../rules/functional-rules';
...
export const isWithinInviteWindow = (now: Date): boolean => {
  ...
  const startMinutes = INVITE_WINDOW_START.hour * 60 + INVITE_WINDOW_START.minute;
  ...
  if (INVITE_WINDOW_START.dayOfWeek === INVITE_WINDOW_END.dayOfWeek) {
    return (
      currentDay === INVITE_WINDOW_START.dayOfWeek && currentMinutes >= startMinutes && currentMinutes <= endMinutes
    );
  }
  ...
  if (INVITE_WINDOW_START.dayOfWeek < INVITE_WINDOW_END.dayOfWeek) {
    return currentDay > INVITE_WINDOW_START.dayOfWeek && currentDay < INVITE_WINDOW_END.dayOfWeek;
  }
...
export const calculateInviteExpiration = (now: Date = new Date(), validityHours = 48): Date => {
