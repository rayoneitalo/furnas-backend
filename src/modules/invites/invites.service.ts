...
import { MAIN_LIST_CAPACITY, MAX_GUESTS_PER_PLAYER } from '../../common/rules/functional-rules';
import { calculateInviteExpiration, isWithinInviteWindow } from '../../common/utils/invite-window.util';
...
export class InvitesService {
  ...
  async createInvite(hostPlayerId: string, dto: CreateInviteDto): Promise<{ token: string; expiresAt: Date }> {
    const now = new Date();
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (host.userId !== dto.hostUserId) {
        throw new ForbiddenException('You are not allowed to generate invites for this player.');
      }
  ...
  async acceptInvite(dto: AcceptInviteDto) {
    ...
    const player = await this.prisma.$transaction(async (tx) => {
      ...
      const mainCount = await tx.player.count({ where: { status: ListStatus.MAIN } });

      const status = mainCount < MAIN_LIST_CAPACITY ? ListStatus.MAIN : ListStatus.WAITLIST;
