...
export class PlayersService {
  ...
  async create(dto: CreatePlayerDto): Promise<PlayerModel> {
    ...
    if (existing) {
      throw new ConflictException('The user already has an active entry in the list.');
    }
  ...
  async remove(playerId: string, dto: RemovePlayerDto): Promise<void> {
    ...
    if (player.userId !== dto.userId) {
      throw new ForbiddenException('You do not have permission to remove this player.');
    }
  ...
  async exportList(): Promise<string> {
    ...
    const lines = [
      ...
      '| --- | --- | --- | --- |',
      ...ordered.map(
        (player) => `| ${player.name} | ${player.profile} | ${player.status} | ${player.isGuest ? 'Sim' : 'Não'} |`,
      ),
    ];
  ...
  async listCurrentPlayers(): Promise<
    Array<{ name: string; isGuest: boolean; invitedBy?: { name: string; userId: string } }>
  > {
    const players = await this.prisma.player.findMany({
      orderBy: [
        { status: 'asc' },
